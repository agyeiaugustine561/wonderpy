...
def create_table(form_container):
    table_frame = tk.Frame(form_container, bg="white")
    table_frame.grid(row=1, column=0, sticky=tk.W+tk.E, pady=20)
...
def add_medicine(entries, table_frame):
    ...
    try:
        ...
        messagebox.showerror("Error", "Quantity and Selling Price must be numeric.")
        return
...
def clear_entries(entries):
    ...
    entries['medicine_name'].var.set("-- Select --")
...
def main():
    ...
    entries, form_container = create_form(main_frame)
    table_frame = create_table(form_container)
