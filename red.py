...
def create_form(main_frame, table_frame):
    ...
    form_container = tk.Frame(main_frame, bg="white", padx=20, pady=20, relief="groove", borderwidth=2)
    ...
    # Configure grid columns and rows for form_frame
    ...
    add_button.grid(row=4, column=0, columnspan=3, pady=10, sticky=tk.W+tk.E)
...
def create_table(main_frame):
    # Create a canvas widget to hold the table and allow scrolling
    table_canvas = tk.Canvas(main_frame, bg="white")
    table_canvas.grid(row=1, column=0, sticky=tk.N+tk.S+tk.E+tk.W)

    # Add a vertical scrollbar
    v_scrollbar = tk.Scrollbar(main_frame, orient="vertical", command=table_canvas.yview)
    ...
    # Set a minimum size for the canvas scrolling region
    table_frame.update_idletasks()  # Let Tkinter calculate table size
    table_canvas.config(scrollregion=table_canvas.bbox("all"))

    # Reconfigure the scroll region whenever the window is resized or new rows are added
...
def add_row(table_frame, date, medicine_name, quantity, selling_price, total_amount):
    row = len(table_frame.grid_slaves()) // 6 + 1  # Calculate the new row index
...
if __name__ == "__main__":
    ...
    # Create the form and table with scrollbar
    form_entries, form_frame = create_form(main_frame, create_table(main_frame))
